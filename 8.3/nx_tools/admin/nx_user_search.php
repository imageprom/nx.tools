...
{
    $f_ID = $arRes['ID'];
    $f_NAME = htmlspecialcharsbx('('.$arRes["LOGIN"].') '.$arRes["NAME"].' '.$arRes["LAST_NAME"]);
    $row =& $lAdmin->AddRow($f_ID, $arRes);
    $row->AddViewField("ID", $f_ID);
    $row->AddCheckField("ACTIVE", false);
    $row->AddViewField("LOGIN", "<a href=\"javascript:SetValue('".$f_ID."', '".$f_NAME."');\" title=\"".GetMessage("MAIN_CHANGE")."\">".$arRes["LOGIN"]."</a>");
