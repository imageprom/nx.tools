...
if ($GLOBALS["install_step"] == 2):
    ...
    //TODO Разобраться с публичной дирректорией
    if (strlen($public_dir) > 0) :
        ...
        <table border="0" cellspacing="0" cellpadding="3">
            ...
            <?
            $sites = CSite::GetList('sort', 'asc', ["ACTIVE" => "Y"]);
            while ($site = $sites->Fetch()) {
                ?>
                <tr>
                    <td width="0%"><p>[<?= $site["ID"] ?>] <?= $site["NAME"] ?></p></td>
                    <td width="0%"><p><a
                                    href="<? if (strlen($site["SERVER_NAME"]) > 0) echo "http://" . $site["SERVER_NAME"]; ?><?= $site["DIR"] . $public_dir ?>/"><?= $site["DIR"] . $public_dir ?>
                                /</a></p></td>
                </tr>
                <?
            }
...
<form action="<?= $APPLICATION->GetCurPage() ?>" name="form1">
    ...
    <p class="nx_tools-install-fields">
    <table cellpadding="3" cellspacing="0" border="0" width="0%">
        <tr>
            <td colspan="2">
                <input type="checkbox" name="install_public" value="Y" id="id_install_public"
                       onclick="ChangeInstallPublic(this.checked)">
                <label for="id_install_public"><?= GetMessage("COPY_PUBLIC_FILES") ?></label></td>
        </tr>
        <tr>
            <td>&nbsp;</td>
            <td>
                <div class="nx_tools-install-field nx_tools-install-field-copy">
                    <label for="public_dir"><?= GetMessage("COPY_FOLDER") ?></label>
                    <input type="input" name="public_dir" id="public_dir" value="update" size="40"/>
                </div>
                <div class="nx_tools-install-field nx_tools-install-field-rewrite">
    ...
    <style>
        p.nx_tools-install-fields table, p.nx_tools-install-fields td, p.nx_tools-install-fields label {
            font-size: 100%;
        }
    </style>
    <script language="JavaScript">
