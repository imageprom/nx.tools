BX.namespace('BX.HLblock');

BX.HLblock.HLblockElementSelector = (function ()
{
	var HLblockElementSelector = function(parameters)
	{
		...
		this.hlblock = parameters.hlblock;
		this.type = parameters.type;

		this.init();
	};

	HLblockElementSelector.prototype.init = function ()
	{
		if(!this.selectorId)
		{
			return false;
		...
		this.ajaxUrl = '/local/components/ip/nx.hlblock.element.selector/ajax.php';
	...
	HLblockElementSelector.prototype.search = function()
	{
		...
		if(this.searchInput.value.length > 0)
		{
			...
			this.url = this.ajaxUrl + '?sessid='+BX.bitrix_sessid()
				+'&mode=search&hlblock='+this.hlblock+'&admin='+this.adminSection+'&string='+
				encodeURIComponent(this.searchInput.value)+'&type='+this.type;
			this.requestTimeout = setTimeout(BX.proxy(this.request, this), 400);
		}
	};

	HLblockElementSelector.prototype.displayTab = function(tab)
	{
	...
	HLblockElementSelector.prototype._onFocus = function()
	{
		this.searchInput.value = '';
	};

	HLblockElementSelector.prototype.showResult = function(data)
	{
			...
			for (var i = 0; i < elements.length; i++)
			{
				...
				elementRow = BX.create('div', {
					props : {
						...
						id: 'ies-anchor_element_id_' + elements[i].ID
					},
					events : {
	...
	HLblockElementSelector.prototype.select = function(event)
	{
		...
		else
		{
			...
			if(inputObject.checked)
			{
				...
				if(!BX(this.selectorId + '_element_selected_' + inputObject.value))
				{
					...
					var elementRow = BX.create('div', {
						props : {
							...
							id: this.selectorId + '_element_selected_' + inputObject.value
						},
						children : [
							BX.create('div', {
								props : {
									id: this.selectorId+'-element-unselect-'+inputObject.value,
									className: 'ies-content-selected-item-icon'
								},
								attrs : {
									onclick:'BX.HLblock["'+this.jsObject+'"].unselect("'+inputObject.value+'");'
								}
							}),
							BX.create('span', {
								props : {
			...
			{
				BX.remove(BX(this.selectorId + '_element_selected_' + inputObject.value));
				BX.remove(BX(this.selectorId+'_selected_value_'+inputObject.value));
				countSpan = BX(this.selectorId + '_current_count');
				countSpan.innerHTML = parseInt(countSpan.innerHTML) - 1;
		...
		if(this.onSelect)
		{
			...
			{
				let lastElement = Object.keys(this.selectedElements).reverse()[0];
				var emp = this.selectedElements[lastElement];
	...
	HLblockElementSelector.prototype.unselect = function(elementId, internalCall)
	{
		...
		if(BX(this.selectorId+'_selected_value_'+elementId))
		{
			BX(this.selectorId+'_selected_value_'+elementId).value = 0;
		}
	...
	HLblockElementSelector.prototype.getSelected = function()
	{
		return this.selectedElements;
	};

	HLblockElementSelector.prototype.setSelected = function(elements)
	{
		var i, count;
		...
		for(i = 0, count = elements.length; i < count; i++)
		{
			...
			if(this.multiple)
			{
				...
				var elementRow = BX.create('div', {
					props : {
						className : 'ies-content-selected-item',
						// id: this.selectorId + '_element_selected_'+elements[i].id
						id: this.selectorId + '_element_selected_1'
					},
					children : [
						BX.create('div', {
							props : {
								id: this.selectorId+'-element-unselect-'+elements[i].id,
								className : 'ies-content-selected-item-icon'
							},
							attrs : {
								onclick : 'BX.HLblock["'+this.jsObject+'"].unselect("'+elements[i].id+'");'
							}
						}),
						BX.create('span', {
							props : {
	...
	HLblockElementSelector.prototype.toObject = function(brokenArray)
	{
		var result = {},
			k;
	...
	HLblockElementSelector.prototype.request = function()
	{
	...
	HLblockElementSelector.prototype.showSelector = function()
	{
		console.log('show');
		if(!this.popupWindow)
		{
	...
	HLblockElementSelector.prototype.setToSelectedValues = function(selectedElements)
	{
			...
			if(selectedElement)
			{
				if(!BX(this.selectorId+'_selected_value_'+selectedElement.id))
				{
					BX(this.selectorId+'_hidden_values').appendChild(
						BX.create('input', {
							props: {
								id: this.selectorId+'_selected_value_'+selectedElement.id
							},
							attrs: {
								type: 'hidden',
				...
				if(!this.multiple && !this.onlyRead)
				{
					listSelectedElements += '<span class="ies-content-delete-icon" onclick="BX.Iblock[\''+this.jsObject+
						'\'].unselect(\''+selectedElement.id+'\')"></span>';
				}
	...
	HLblockElementSelector.prototype.getCountElements = function(elements)
	{
		var count = 0, i;
