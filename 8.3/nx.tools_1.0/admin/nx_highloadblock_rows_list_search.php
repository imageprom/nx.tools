...
//	$row->AddActions($arActions);
    ...
    $row->AddActions(array(
        array(
            "DEFAULT" => "Y",
            "TEXT" => 'Выбрать',
            "ACTION"=>"javascript:SelEl('".$SelElVals."', '".htmlspecialcharsbx($jsPath.htmlspecialcharsbx(CUtil::JSEscape($arRes["UF_NAME"]), ENT_QUOTES)).$nameSeparator."', '".$arRes['ID']."')",
        ),
    ));
...
$reloadParams['ENTITY_ID'] =$hlblock['ID'];
...
?>
    <script type="text/javascript">
        ...
        function applyFilter(el)
        {
            if (blockedFilter)
                return false;
            <?=$sTableID."_filter_id";?>.OnSet('<?=CUtil::JSEscape($sTableID); ?>', '<?=CUtil::JSEscape($reloadUrl); ?>');
            return false;
        }
        function deleteFilter(el)
        {
            ...
            var tbl = document.querySelectorAll('#<?=$sTableID."_filter_id_content"?> input');
            ...
            <?=$sTableID."_filter_id"?>.OnClear('<?=CUtil::JSEscape($sTableID); ?>', '<?=CUtil::JSEscape($reloadUrl)?>');
            return false;
        }


        function SelEl(xml_id, name, id)
        {
            var el;
            var parent;
            <?
                if ('' != $lookup)
                {
                    if ('' != $m)
                {
                ?>window.opener.<? echo $lookup; ?>.AddValue(xml_id);<?
            }
            else
            {
            ?>
        ...
        if($m)
        {
        ?>window.opener.InS<?echo md5($n)?>(xml_id, name, id);<?
            }
            else
            {
            ...
            el =  parent.querySelector('span.name_element');
            if(el)
